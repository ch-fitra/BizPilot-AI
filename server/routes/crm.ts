...
router.get('/dashboard', async (req, res) => {
  try {
    ...
    res.status(err.status || 500).json({ success: false, error: err.message || 'Gagal mengompilasi statistik CRM.' });
  }
});
...
router.get('/leads', async (req, res) => {
  try {
    ...
    res.status(err.status || 500).json({ success: false, error: err.message || 'Gagal memuat list prospek.' });
  }
});
...
router.put('/leads/:id', enforceRole('edit'), async (req, res) => {
  try {
    ...
    const result = await CRMLeadRepository.updateLead(req.params.id, sanitized, activeBusinessId);
    if (!result) {
...
router.delete('/leads/:id', enforceRole('delete'), async (req, res) => {
  try {
    ...
    const success = await CRMLeadRepository.deleteLead(req.params.id, activeBusinessId);
    if (!success) {
      return res.status(404).json({ success: false, error: 'Prospek Lead tidak ditemukan atau gagal dihapus.' });
    }
